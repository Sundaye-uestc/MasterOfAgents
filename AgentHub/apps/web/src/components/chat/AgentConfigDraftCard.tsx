...
export function AgentConfigDraftCard({ draft, onConfirm, onCancel }: AgentConfigDraftCardProps) {
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 my-3 max-w-[75%]">
      <p className="text-sm font-medium text-gray-200 mb-3">创建新 Agent？</p>

      <div className="space-y-2 mb-3 text-sm">
        <div className="flex justify-between">
          <span className="text-gray-500">名称</span>
          <span className="text-gray-200">{draft.name}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-500">平台</span>
          <span className="text-gray-200">{draft.platform}</span>
        </div>
        {draft.systemPrompt && (
          <div>
            <span className="text-gray-500 text-xs">System Prompt</span>
            <p className="text-gray-400 text-xs mt-0.5 line-clamp-2">{draft.systemPrompt}</p>
          </div>
        )}
