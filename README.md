...
Node.js >= 20，pnpm >= 9，Python >= 3.10（可选），Claude Code CLI（必选），Codex CLI（可选），Open Code CLI（可选）
...
![Agent Settings](imgs/agent-settings.png)

### Workspace

实时文件树、每次 Agent 运行自动快照、彩色 Unified Diff（+ 绿 / - 红 / 修改 黄）、文件变更追踪与一键回滚、面板宽度可拖拽（240–600px）。
