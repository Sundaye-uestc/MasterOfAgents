...
- [ ] 文件树数据为空 — WorkspacePanel 的 files 始终传 `[]`，从未真正加载工作区文件树
- [ ] 快照列表为空 — snapshots 始终传 `[]`，未从 workspace.store 加载
- [ ] 实时同步不完整 — ChatArea 通过 `onFileChangesSync` 回调同步 fileChanges，但初始加载与 WS 更新可能存在时序问题
- [ ] workspace.store 未接入 App — 已有 `useWorkspaceStore` 和 `load()` 方法但未在 App 中使用
- [ ] 文件树构建 — 需要实现从 `rootPath` 递归扫描文件构建 `FileNode[]` 的逻辑（前端或后端）
- [ ] 文件树选中状态 — 当前 `selectedPath` 由 panel 内部管理，未与编辑器/预览联动
- [ ] 面板可见性控制 — 缺少切换按钮，无法按需显示/隐藏右侧面板
- [ ] 后端确保 manifest 数据正确生成 — workspace 的 manifest/spanshots 链路需端到端验证

### 当前行为
WorkspacePanel 已渲染在界面右侧（320px），但文件 Tab 显示"工作区为空"，快照 Tab 显示"暂无快照"，仅有变更 Tab 能够展示 file_changes 数据。
