...
| Commit | 说明 |
|---|---|
| 待提交 | Feat: mobile MVP — scaffold, navigation, network detection |
| 待提交 | Feat: mobile conversation list + chat page with WebSocket |
| 待提交 | Feat: mobile approval panel + artifact preview + settings |
| 待提交 | Feat: backend profile API + shared assets + multi-client avatar sync |
| 待提交 | Fix: start_dev.py DETACHED_PROCESS + separate web/mobile launchers |
